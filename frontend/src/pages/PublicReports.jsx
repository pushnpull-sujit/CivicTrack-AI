...
import { 
  ...
  Sparkles, CheckCircle, Eye, Activity 
} from 'lucide-react';
...
export default function PublicReports() {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      console.error(err);
      showToast('Error', 'Failed to retrieve public complaints feed.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 text-slate-900 dark:text-slate-100">
      <div className="rounded-[28px] border border-white bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-[#0f172a]/70 backdrop-blur-xl">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5 dark:border-slate-800">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Public City Reports</h2>
            <p className="text-sm text-slate-500 dark:text-white/60">Browse community reported issues and verify resolving actions across your city.</p>
          </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-4">
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs outline-none dark:border-slate-800 dark:bg-slate-900 dark:text-slate-350"
          >
        ...
        {loading ? (
          <div className="py-20 text-center text-slate-400">
            ...
            Loading city feed database...
          </div>
        ) : filteredComplaints.length === 0 ? (
          <div className="py-20 text-center text-slate-400">
            <p className="text-base font-bold text-slate-700 dark:text-slate-300">No public reports found</p>
            <p className="text-xs mt-1">Check back later or change filter preferences.</p>
          </div>
        ) : (
          <div className="mt-6 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            {filteredComplaints.map((cmp) => (
              <div 
                key={cmp._id}
                className="overflow-hidden rounded-3xl border border-slate-100 bg-slate-50/30 shadow-sm transition hover:shadow-md dark:border-slate-850 dark:bg-slate-900/40 flex flex-col"
              >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-xl bg-violet-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-violet-700 transition"
                    >
                      <Eye size={12} />
                      View details
                    </button>
